...
async def manual_getter(message: types.Message):
    ...
    await message.answer(   
        'Ниже приведена инструкция по использованию различных режимов ChatGPT:\n1. Режим ChatGPT (режим по умолчанию):\n Просто напишите что-то, не выбирая никаких опций. Бот будет использовать модель ChatGPT 3.5 Turbo, которая является одной из самых передовых моделей для генерации текста на рынке. Она способна предоставить высококачественные ответы на широкий спектр вопросов, которые вы ей зададите.\n\n'
        'Режим генерации изображений (DALL-E):\n 2. Выберите этот режим, а затем напишите запрос о том, что вы хотите увидеть на изображении. Бот будет использовать модель DALL-E, которая способна генерировать уникальные и красивые изображения на основе ваших запросов.\n\n'
